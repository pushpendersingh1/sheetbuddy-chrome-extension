...
describe('AssemblyAIAdapter', () => {
  ...
  describe('close()', () => {
    it('sends terminate_session over the WebSocket when OPEN', () => {
      adapter.close();
      expect(WebSocketStub.lastInstance.send).toHaveBeenCalledWith(
    ...
    it('does not send if WebSocket is already CLOSED', () => {
      WebSocketStub.lastInstance.readyState = WebSocketStub.CLOSED;
      adapter.close();
      expect(WebSocketStub.lastInstance.send).not.toHaveBeenCalled();
    });
    ...
    it('closes a CONNECTING WebSocket without sending terminate_session', () => {
      WebSocketStub.lastInstance.readyState = 0; // WebSocket.CONNECTING
      adapter.close();
      expect(WebSocketStub.lastInstance.close).toHaveBeenCalled();
      expect(WebSocketStub.lastInstance.send).not.toHaveBeenCalled();
    });
